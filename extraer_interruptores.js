...
async function main() {
  const facetRes = await search({
    query: '',
    filters: { all: [{ classification_l2: 'Aparellaje eléctrico' }, { classification_l3: 'Interruptor y conmutador manual' }]},
  ...
  for (const m of marcas) {
    ...
    const arts = await getAll([
      ...
      { classification_l3: 'Interruptor y conmutador manual' },
      { brand_name: m.value }
    ], m.value);
    todos.push(...arts);
  }
  ...
  fs.writeFileSync('interruptores_completo.json', JSON.stringify(unicos, null, 2));

  const porClase = {};
  unicos.forEach(a => {
    const cl = (Array.isArray(a.etim_class_name) ? a.etim_class_name[0] : a.etim_class_name) || 'SIN_CLASE';
    if (!porClase[cl]) porClase[cl] = { count: 0, marcas: {} };
    porClase[cl].count++;
    const marca = a.brand_name || '?';
    porClase[cl].marcas[marca] = (porClase[cl].marcas[marca] || 0) + 1;
  });

  console.log('\n=== CLASES ETIM ===');
  Object.entries(porClase).sort((a, b) => b[1].count - a[1].count).forEach(([cl, data]) => {
    const ms = Object.entries(data.marcas).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([m,n])=>`${m}(${n})`).join(', ');
    console.log(`${cl} | ${data.count} | ${ms}`);
  });
}

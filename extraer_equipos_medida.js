...
async function main() {
  const facetRes = await search({
    query: '',
    filters: { all: [{ classification_l2: 'Aparellaje eléctrico' }, { classification_l3: 'Equipos de medida (panel o carril DIN)' }]},
  ...
  for (const m of marcas) {
    ...
    const arts = await getAll([
      ...
      { classification_l3: 'Equipos de medida (panel o carril DIN)' },
      { brand_name: m.value }
    ], m.value);
    todos.push(...arts);
  }
  ...
  fs.writeFileSync('equipos_medida_completo.json', JSON.stringify(unicos, null, 2));

...
async function main() {
  const facetRes = await search({
    query: '',
    filters: { all: [{ classification_l2: 'Aparellaje eléctrico' }, { classification_l3: 'Otros aparatos' }]},
  ...
  for (const m of marcas) {
    ...
    const arts = await getAll([
      ...
      { classification_l3: 'Otros aparatos' },
      { brand_name: m.value }
    ], m.value);
    todos.push(...arts);
  }
  ...
  fs.writeFileSync('otros_aparatos_completo.json', JSON.stringify(unicos, null, 2));

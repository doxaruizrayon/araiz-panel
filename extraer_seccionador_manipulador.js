...
async function main() {
  const facetRes = await search({
    query: '',
    filters: { all: [{ classification_l2: 'Aparellaje eléctrico' }, { classification_l3: 'Seccionador y manipulador manual' }]},
  ...
  for (const m of marcas) {
    ...
    const arts = await getAll([
      ...
      { classification_l3: 'Seccionador y manipulador manual' },
      { brand_name: m.value }
    ], m.value);
    todos.push(...arts);
  }
  ...
  fs.writeFileSync('seccionador_manipulador_completo.json', JSON.stringify(unicos, null, 2));

...
async function main() {
  const facetRes = await search({
    query: '',
    filters: { all: [{ classification_l2: 'Aparellaje eléctrico' }, { classification_l3: 'Disyuntores y guardamotores' }]},
  ...
  for (const m of marcas) {
    ...
    const arts = await getAll([
      ...
      { classification_l3: 'Disyuntores y guardamotores' },
      { brand_name: m.value }
    ], m.value);
    todos.push(...arts);
  }
  ...
  fs.writeFileSync('disyuntores_guardamotores_completo.json', JSON.stringify(unicos, null, 2));
